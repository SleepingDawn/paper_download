...
## Linux seeded profile 준비

`runtime-preset=linux_cli_seeded`를 제대로 쓰려면 Linux에서 만든 Chrome `user-data-dir` 루트가 필요합니다. macOS 시스템 Chrome profile을 그대로 넘기면 현재 코드 경로와 맞지 않습니다.

권장 절차는:

1. 맥북에서 Ubuntu VM을 띄움
2. VM 안에서 전용 Chrome `user-data-dir` 생성 후 기관 인증 + ScienceDirect/PDF 몇 건으로 warm up
3. `scripts/check_linux_seed_profile.py`로 구조 확인
4. `scripts/package_linux_seed_profile.py`로 tar bundle 생성
5. 서버에 복사 후 `--persistent-profile-dir`로 연결
