...
def summarize_metrics(records: List[DownloadAttempt]) -> Dict:
    if not records:
        return {
            "count": 0,
            "success_rate": 0.0,
    ...
    n = len(latencies)
    median = latencies[n // 2] if n % 2 == 1 else int((latencies[n // 2 - 1] + latencies[n // 2]) / 2)
    p90_idx = min(n - 1, int(n * 0.9))
    p90 = latencies[p90_idx]

    success_count = sum(1 for r in records if r.success)
    false_positive = sum(1 for r in records if r.success and r.reason != REASON_SUCCESS)
    ...
    for key, target in (("strategy", by_strategy), ("domain", by_domain)):
        groups: Dict[str, List[DownloadAttempt]] = {}
        for r in records:
            gk = getattr(r, key)
        ...
        for gk, vals in groups.items():
            target[gk] = {
                "count": len(vals),
                "success_rate": round(sum(1 for v in vals if v.success) / len(vals), 4),
                "median_latency_ms": sorted(v.elapsed_ms for v in vals)[len(vals) // 2],
            }
