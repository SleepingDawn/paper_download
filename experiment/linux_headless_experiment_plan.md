...
- Sci-Hub가 성공률을 가리는 것을 막기 위해, 기본 검증 세트는 `publication_year >= 2024`를 우선 선택한다.
...
- `pilot` 13건
  ...
  - 가능한 한 `2024+`를 우선 선택하고, 부족한 경우에만 `legacy_fallback`을 허용
...
- `full` 31건
  ...
  - `2024+`를 기본 검증 집합으로 삼고, 최근 DOI가 모자란 publisher만 fallback으로 채운다.
  - 목적: publisher별 landing/download 차이를 실제로 비교

### 최근성 / Sci-Hub confound 관리

- `publication_year >= 2024`
  - `validation_cohort=recent_primary`
  - `scihub_confound_risk=lower_recent`
  - publisher-native 후속 다운로드 로직 검증의 메인 분모
- `publication_year < 2024`
  - `validation_cohort=legacy_fallback`
  - `scihub_confound_risk=higher_legacy`
  - publisher 다양성 유지를 위한 보조 분모
- `cell`과 `rsc`는 recent DOI를 우선 고르고, 부족하면 fallback을 명시적으로 남긴다.
...
- 성공 기준:
  - `download_status`가 success이면서 source path가 구분돼야 한다.
  - `download_source_category == publisher_native`면 publisher-native 성공
  - `download_source_category == scihub_assisted`면 Sci-Hub-assisted 성공
  - source path를 알 수 없으면 `download_success_unknown`으로 별도 집계
...
- download run은 `parallel_download.py`가 기록하는 `download_method`, `download_source_category`, `experiment_landing_bucket`, `experiment_download_bucket`, `download_result_reason`, `download_result_stage`를 사용한다.
...
4. landing과 download가 분리 관측되는가
   ...
   - Sci-Hub-assisted 성공은 publisher-native 성공과 다른 bucket으로 분리돼야 한다.
...
1. `parallel_download.py`의 `download_method`, `download_source_category`, `experiment_*_bucket`, `download_result_*` 컬럼이 publisher-native vs Sci-Hub를 충분히 분리하는지
